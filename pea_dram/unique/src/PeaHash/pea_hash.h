...
// 8B
    ...
    struct function {
        ...
        int (*opt_insert)(T key, Value_t value, uint64_t hash, bucket_t *first_bucket,
                          uint32_t chunk_id, seg_meta *segment, uint32_t old_dir_lock_value,
                          uint32_t old_seg_lock_value);
    ...
    public:
        ...
        static inline bool is_locked_by(uint32_t lock_id) {
            uint32_t *lock = (uint32_t *) &lock_map_table[lock_id];
            uint32_t value = __atomic_load_n(lock, __ATOMIC_ACQUIRE);
            lock_type v = *((lock_type *) &value);
            return (v.ref_count > 0);
...
// release and flip lock
    ...
    PeaHashing<T>::PeaHashing(size_t initCap, uint64_t t_num) {
        ...
        *global_depth = G;
        ...
        dir_lock = 0;
        lock_table_size = LOCK_SIZE_PER_SEG * (1UL << (G + 2));
        lock_map_table = (lock_type *) malloc(lock_table_size * sizeof(lock_type));
        memset(lock_map_table, 0, lock_table_size * sizeof(lock_type));
        thread_num = t_num;
...
//        if (hashPolicy::is_locked_by(292)) {
//            LOG_FATAL("292 here locked");
//        }
        switch (ret) {
            case -4:
                goto RETRY;
 ...
 */
    ...
    void PeaHashing<T>::Directory_Doubling(uint32_t idx, seg_meta *new_s) {
        ...
        *global_depth = old_G + 2;
        dir = new_d;
        uint64_t old_lock_table_size = lock_table_size;
        lock_table_size = lock_table_size << 2UL;
        void *new_lock_mem = realloc(lock_map_table, lock_table_size * sizeof(lock_type));
        if (!new_lock_mem) {
            LOG_FATAL("memory is exhausted");
        }
        lock_map_table = (lock_type *)new_lock_mem;
        memset(lock_map_table + old_lock_table_size, 0, old_lock_table_size * 3 * sizeof(lock_type));
    }
 ...
 */
    ...
    public:
        ...
        static int opt_insert(T key, Value_t value, uint64_t hash, bucket_t *first_bucket,
                              uint32_t chunk_id, seg_meta *target_seg, uint32_t old_dir_lock_value,
                              uint32_t old_seg_lock_value) {
            bucket_t *bucket;
            uint32_t bucket_id = bucket_idx0(hash);
            bucket = &first_bucket[bucket_id];
