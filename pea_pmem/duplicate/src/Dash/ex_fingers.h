...
struct ValueArray {
  ...
  static void New(PMEMoid *arr, size_t capacity) {
    auto callback = [](PMEMobjpool *pool, void *ptr, void *arg) {
      auto *cap_ptr = reinterpret_cast<size_t *>(arg);
      auto arr_ptr = reinterpret_cast<ValueArray *>(ptr);
      arr_ptr->number = 0;
      arr_ptr->cap = *cap_ptr;
      pmemobj_persist(pool, arr_ptr,
                      sizeof(uint64_t) * 2 + sizeof(Value_t) * (*cap_ptr));
      return 0;
    };
    size_t *callback_args = &capacity;
    Allocator::Allocate(arr, kCacheLineSize,
                        sizeof(uint64_t) * 2 + sizeof(Value_t) * capacity,
                        callback, callback_args);
#ifdef AU
    segment_num += (2 + capacity / 2);
#endif
  }

  int CopyFromAnother(ValueArray *src) {
    this->number = src->number;
    memcpy(this->_, src->_, sizeof(Value_t) * (src->number));
    Allocator::Persist(this,
                       sizeof(uint64_t) * 2 + sizeof(Value_t) * (src->number));
    return 0;
  }

  size_t Insert(Value_t val, PMEMobjpool *pool) {
    if (number == cap) {
      return cap;
    }
    this->_[number] = val;
    Allocator::Persist(&_[number], sizeof(_[number]));
    number++;
    Allocator::Persist(&number, sizeof(number));
    return 0;
  }

  int Delete() {
    auto reserve_item = Allocator::ReserveItem();
    Allocator::Free(reserve_item, this);
#ifdef AU
    segment_num -= (2 + cap / 2);
#endif
    return 0;
  ...
  int Get(Value_t *ret_val, size_t *val_num) {
    if (*val_num < number) {
      return -1;
    } else {
#ifdef ANA
      // if(temp == 0) LOG((uint64_t *)this);
      // switch (number)
      // {
      // case 1: val1cnt++;
      //   break;
      ...
      // }
#endif
      memcpy(ret_val, this->_, number * sizeof(Value_t));
      *val_num = number;
      return 0;
    }
  }
...
struct Bucket {
  ...
  /**
   * @return 0: find duplicate and insert it. -1: not find any duplicate
   */
  int unique_check_insert(uint8_t meta_hash, T key, Value_t val,
                          Bucket<T> *neighbor, Bucket<T> *stash,
                          PMEMobjpool *pool) {
    if ((check_and_insert(meta_hash, key, val, false, pool) != -2) ||
        (neighbor->check_and_insert(meta_hash, key, val, true, pool) != -2)) {
      return 0;
    }
    ...
    if (test_stash_check()) {
      ...
      if (test_stash == true) {
        for (int i = 0; i < stashBucket; ++i) {
          Bucket *curr_bucket = stash + i;
          if (curr_bucket->check_and_insert(meta_hash, key, val, false, pool) !=
              -2) {
            return 0;
          }
        }
  ...
  int check_and_get_multi(uint8_t meta_hash, T key, bool probe, Value_t *array,
                          size_t *number) {
    int mask = 0;
    SSE_CMP8(finger_array, meta_hash);
    if (!probe) {
    ...
    if constexpr (std::is_pointer_v<T>) {
      ...
      for (int i = 0; i < 14; i += 1) {
        if (CHECK_BIT(mask, i) &&
            (var_compare((reinterpret_cast<string_key *>(_[i].key))->key,
                         ...
                         (reinterpret_cast<string_key *>(_[i].key))->length,
                         _key->length))) {
          return value2pointer_lookup(_[i].value, array, number);
        }
      }
    } else {
      /*fixed-length key*/
      ...
      for (int i = 0; i < 12; i += 4) {
        ...
        if (CHECK_BIT(mask, i + 1) && (_[i + 1].key == key)) {
          return value2pointer_lookup(_[i + 1].value, array, number);
        }

        if (CHECK_BIT(mask, i + 2) && (_[i + 2].key == key)) {
          return value2pointer_lookup(_[i + 2].value, array, number);
        }

        if (CHECK_BIT(mask, i + 3) && (_[i + 3].key == key)) {
          return value2pointer_lookup(_[i + 3].value, array, number);
        }
      }

      if (CHECK_BIT(mask, 12) && (_[12].key == key)) {
        return value2pointer_lookup(_[12].value, array, number);
      }

      if (CHECK_BIT(mask, 13) && (_[13].key == key)) {
        return value2pointer_lookup(_[13].value, array, number);
      }
    }
    return -2;
  }
  ...
  /**
   * @return -2: no duplicate; 0 successfully insert to the pointer
   */
  int check_and_insert(uint8_t meta_hash, T key, Value_t val, bool probe,
                       PMEMobjpool *pool) {
    int mask = 0;
    SSE_CMP8(finger_array, meta_hash);
    if (!probe) {
    ...
    if constexpr (std::is_pointer_v<T>) {
      ...
      for (int i = 0; i < 14; i += 1) {
        if (CHECK_BIT(mask, i) &&
            (var_compare((reinterpret_cast<string_key *>(_[i].key))->key,
                         ...
                         (reinterpret_cast<string_key *>(_[i].key))->length,
                         _key->length))) {
          return value2pointer_insert(&_[i].value, val, pool);
        }
      }
    } else {
      /*fixed-length key*/
      ...
      for (int i = 0; i < 12; i += 4) {
        ...
        if (CHECK_BIT(mask, i + 1) && (_[i + 1].key == key)) {
          return value2pointer_insert(&_[i + 1].value, val, pool);
        }

        if (CHECK_BIT(mask, i + 2) && (_[i + 2].key == key)) {
          return value2pointer_insert(&_[i + 2].value, val, pool);
        }

        if (CHECK_BIT(mask, i + 3) && (_[i + 3].key == key)) {
          return value2pointer_insert(&_[i + 3].value, val, pool);
        }
      }

      if (CHECK_BIT(mask, 12) && (_[12].key == key)) {
        return value2pointer_insert(&_[12].value, val, pool);
      }

      if (CHECK_BIT(mask, 13) && (_[13].key == key)) {
        return value2pointer_insert(&_[13].value, val, pool);
      }
    }
    return -2;
  }

  int value2pointer_insert(Value_t *ptr_val, Value_t val, PMEMobjpool *pool) {
    auto *this_arr =
        reinterpret_cast<ValueArray *>(const_cast<char *>(*ptr_val));
    size_t ret = this_arr->Insert(val, pool);
    if (ret) {
      PMEMoid pointer;
      ValueArray::New(&pointer, ret * 2);
      auto new_arr = reinterpret_cast<ValueArray *>(pmemobj_direct(pointer));
      new_arr->CopyFromAnother(this_arr);
      new_arr->Insert(val, pool);
      *ptr_val = reinterpret_cast<Value_t>(new_arr);
#ifdef PMEM
      Allocator::Persist(ptr_val, sizeof(Value_t));
#endif
      this_arr->Delete();
    }
    return 0;
  ...
  int value2pointer_delete(Value_t ptr_val) {
    auto *this_arr =
        reinterpret_cast<ValueArray *>(const_cast<char *>(ptr_val));
    return this_arr->Delete();
  }

  int value2pointer_lookup(Value_t ptr_val, Value_t *array, size_t *num) {
    auto *this_arr =
        reinterpret_cast<ValueArray *>(const_cast<char *>(ptr_val));
    return this_arr->Get(array, num);
  }
  ...
  int InsertUnique(T key, Value_t value, uint8_t meta_hash, bool probe,
                   PMEMobjpool *pool) {
    ...
    PMEMoid pointer;
    ValueArray::New(&pointer, VAL_NUM);
    _[slot].key = key;
    _[slot].value = reinterpret_cast<Value_t>(pmemobj_direct(pointer));
    value2pointer_insert(&_[slot].value, value, pool);
#ifdef PMEM
      ...
      if (mask != 0) {
        for (int i = 0; i < 12; i += 4) {
          if (CHECK_BIT(mask, i) &&
              (var_compare((reinterpret_cast<string_key *>(_[i].key))->key,
                           ...
                           (reinterpret_cast<string_key *>(_[i].key))->length,
                           _key->length))) {
            value2pointer_delete(_[i].value);
            unset_hash(i, false);
            return 0;
          }
          ...
          if (CHECK_BIT(mask, i + 1) &&
              (var_compare(
                  ...
                  _key->length))) {
            value2pointer_delete(_[i + 1].value);
          ...
          if (CHECK_BIT(mask, i + 2) &&
              (var_compare(
                  ...
                  _key->length))) {
            value2pointer_delete(_[i + 2].value);
          ...
          if (CHECK_BIT(mask, i + 3) &&
              (var_compare(
                  ...
                  _key->length))) {
            value2pointer_delete(_[i + 3].value);
        ...
        if (CHECK_BIT(mask, 12) &&
            (var_compare(reinterpret_cast<string_key *>(_[12].key)->key,
                         ...
                         _key->length))) {
          value2pointer_delete(_[12].value);
          unset_hash(12, false);
          return 0;
        }
        ...
        if (CHECK_BIT(mask, 13) &&
            (var_compare(reinterpret_cast<string_key *>(_[13].key)->key,
                         ...
                         _key->length))) {
          value2pointer_delete(_[13].value);
          unset_hash(13, false);
          return 0;
        }
      }
  ...
  void Insert_displace(T key, Value_t value, uint8_t meta_hash, int slot,
    ...
    ValueArray::New(&pointer, VAL_NUM);
    _[slot].value = reinterpret_cast<Value_t>(pmemobj_direct(pointer));
    value2pointer_insert(&_[slot].value, value, nullptr);
    _[slot].key = key;
#ifdef PMEM
  ...
  inline size_t getNumber() {
    ...
    for (int j = 0; j < kNumPairPerBucket; ++j) {
      if (CHECK_BIT(mask, j)) {
        auto *this_arr =
            reinterpret_cast<ValueArray *>(const_cast<char *>(_[j].value));
#ifdef AU
        uint32_t arr_cnt = this_arr->number;
  ...
  uint8_t finger_array[18]; /*only use the first 14 bytes, can be accelerated
                               by SSE instruction,0-13 for finger, 14-17 for
                               overflowed*/
  uint8_t overflowBitmap;
  uint8_t overflowIndex;
  uint8_t overflowMember; /*overflowmember indicates membership of the
...
#ifdef AU
    segment_num +=
        (sizeof(Directory<T>) + sizeof(table_p) * capacity + 15) / 16;
#endif
#else
...
#ifdef AU
    segment_num += (sizeof(Table<T>) * kAllTables + 15) / 16;
#endif
...
struct Table {
  static void New(PMEMoid *tbl, size_t depth, PMEMoid pp) {
    ...
    *tbl = pmemobj_oid(ptr);
    ...
    Allocator::Allocate(tbl, kCacheLineSize, sizeof(Table<T>), callback,
                        reinterpret_cast<void *>(&callback_para));
#ifdef AU
    segment_num += (sizeof(Table<T>) + 15) / 16;
    real_seg_num += 1;
#endif
#endif
#else
  ...
  void Insert4splitWithCheck(T key, Value_t value, size_t key_hash,
                             uint8_t meta_hash,
                             PMEMobjpool *); /*with uniqueness check*/
  void Insert4merge(T key, Value_t value, size_t key_hash, uint8_t meta_hash,
                    bool flag = false, PMEMobjpool *pool = nullptr);
  Table<T> *Split(size_t, PMEMobjpool *);
  void HelpSplit(Table<T> *, PMEMobjpool *);
  void Merge(Table<T> *, bool flag = false, PMEMobjpool *pool = nullptr);
  ...
  int Next_displace(Bucket<T> *target, Bucket<T> *neighbor,
                    ...
                    uint8_t meta_hash, PMEMobjpool *pool) {
  ...
  void recoverMetadata() {
    ...
    /*scan the stash buckets and re-insert the overflow FP to initial
     * buckets*/
    for (int i = 0; i < stashBucket; ++i) {
  ...
  int state;          /*-1 means this bucket is merging, -2 means this bucket is
                         ...
                         bucket (NEW)*/
  PMEMmutex lock_bit; /* for the synchronization of the lazy recovery in one
                         segment*/
};
...
int Table<T>::Insert(T key, Value_t value, size_t key_hash, uint8_t meta_hash,
                     Directory<T> **_dir, PMEMobjpool *pool) {
  RETRY:
  ...
  auto ret = target->unique_check_insert(meta_hash, key, value, neighbor,
                                         bucket + kNumBucket, pool);
  if (ret == 0) {
    neighbor->release_lock();
    target->release_lock();
    return 0; /* duplicate insert*/
  }
  ...
  if (GET_COUNT(target->bitmap) <= GET_COUNT(neighbor->bitmap)) {
    target->InsertUnique(key, value, meta_hash, false, pool);
    target->release_lock();
#ifdef PMEM
    Allocator::Persist(&target->bitmap, sizeof(target->bitmap));
#endif
    ...
    neighbor->InsertUnique(key, value, meta_hash, true, pool);
    neighbor->release_lock();
#ifdef PMEM
    Allocator::Persist(&neighbor->bitmap, sizeof(neighbor->bitmap));
#endif
...
void Table<T>::Insert4splitWithCheck(T key, Value_t value, size_t key_hash,
  ...
  if (GET_COUNT(insert_target->bitmap) < kNumPairPerBucket) {
    ...
    Stash_insert(target, neighbor, key, value, meta_hash, y & stashMask, pool);
  }
}

/*the insert needs to be perfectly balanced, not destory the power of
 * balance*/
template <class T>
...
template <class T>
void Table<T>::HelpSplit(Table<T> *next_table, PMEMobjpool *pool) {
      ...
      if (CHECK_BIT(mask, j)) {
        ...
        if ((key_hash >> (64 - local_depth - 1)) == new_pattern) {
          ...
          next_table->Insert4splitWithCheck(curr_bucket->_[j].key,
                                            ...
                                            curr_bucket->finger_array[j], pool);
#ifdef COUNTING
          number--;
#endif
        }
...
template <class T>
Table<T> *Table<T>::Split(size_t _key_hash, PMEMobjpool *pool) {
  size_t new_pattern = (pattern << 1) + 1;
  size_t old_pattern = pattern << 1;
      ...
      if (CHECK_BIT(mask, j)) {
        ...
        if ((key_hash >> (64 - local_depth - 1)) == new_pattern) {
          ...
          next_table->Insert4split(
              ...
              curr_bucket->finger_array[j], pool); /*this shceme may destory the
                                                balanced segment*/
          // curr_bucket->unset_hash(j);
#ifdef COUNTING
          number--;
      ...
      for (int j = 0; j < kNumPairPerBucket; ++j) {
        if (CHECK_BIT(mask, j)) {
          ...
          Insert4merge(curr_bucket->_[j].key, curr_bucket->_[j].value, key_hash,
                       curr_bucket->finger_array[j], true,
                       pool); /*this shceme may destory
 ...
 public:
  ...
  uint64_t getNumber() {
    ...
    for (int i = 0; i < capacity;) {
      ...
      for (int j = 0; j < kNumBucket + stashBucket; ++j) {
        auto curr_bucket = ss->bucket + j;
        // std::cout << i << " " << j << " " << curr_bucket->getNumber() <<
        // std::endl;
        _count += curr_bucket->getNumber();
      }
      seg_count++;
      i += pow(2, depth_diff);
    }
    ...
    std::cout << "current size in entry = " << segment_num << std::endl;
    ...
    for (int i = 0; i < 32; ++i) {
      std::cout << std::setprecision(14) << type_avg[i] << ", ";
    }
    std::cout << std::endl;
#endif
#ifdef COUNTING
  ...
  Directory<T> *dir;
  uint32_t lock;  // the MSB is the lock bit; remaining bits are used as the
  // counter
  uint64_t
...
#ifdef AU
          segment_num +=
        2 * (sizeof(*reserve_item) + sizeof(dir) + sizeof(back_dir) + 15) / 16;
#endif
...
#ifdef AU
          segment_num += 2 *
                   (sizeof(*reserve_item) + sizeof(dir) + sizeof(back_dir) +
                    sizeof(old_b->local_depth) + 15) /
                   16;
#endif
          old_b->local_depth += 1;
          Allocator::Free(reserve_item, dir);
...
#ifdef AU
              segment_num +=
            2 * (15 + sizeof(Table<T> *) + sizeof(old_b->local_depth)) / 16;
#endif
...
#ifdef AU
            segment_num +=
          2 * (15 + sizeof(Table<T> *) * base + sizeof(old_b->local_depth)) /
          16;
#endif
  ...
  if (target->state != 0) {
    ...
    if (target->state == -2) {
      if (next_table->state == -3) {
        ...
        target->HelpSplit(next_table, pool_addr);
        Lock_Directory();
      ...
      if (next_table->pattern == ((target->pattern << 1) + 1)) {
        target->Merge(next_table, true, pool_addr);
...
int Finger_EH<T>::Insert0(T key, Value_t value) {
  ...
  auto ret = target->Insert(key, value, key_hash, meta_hash, &dir, pool_addr);
  ...
  if (ret == -1) {
    ...
    auto new_b = target->Split(
        key_hash, pool_addr); /* also needs the verify..., and we use try
...
template <class T>
int Finger_EH<T>::Get(T key, Value_t *ret_val, size_t *number) {
  uint64_t key_hash;
  if constexpr (std::is_pointer_v<T>) {
    key_hash = h(key->key, key->length);
  } else {
  ...
  auto ret = target_bucket->check_and_get_multi(meta_hash, key, false, ret_val,
                                                number);
  ...
  ret = neighbor_bucket->check_and_get_multi(meta_hash, key, true, ret_val,
                                             number);
  ...
  if (target_bucket->test_stash_check()) {
      ...
      if (mask != 0) {
        for (int i = 0; i < 4; ++i) {
          if (CHECK_BIT(mask, i) &&
            ...
            auto ret = stash->check_and_get_multi(meta_hash, key, false,
                                                  ret_val, number);
            if (ret != -2) {
              if (target_bucket->test_lock_version_change(old_version)) {
                goto RETRY;
    ...
    if (test_stash == true) {
      for (int i = 0; i < stashBucket; ++i) {
        ...
        auto ret =
            stash->check_and_get_multi(meta_hash, key, false, ret_val, number);
        if (ret != -2) {
          if (target_bucket->test_lock_version_change(old_version)) {
            goto RETRY;
          }
    ...
    if (left_seg->Acquire_and_verify(_pattern0)) {
      if (right_seg->Acquire_and_verify(_pattern1)) {
        ...
        if (right_seg->number != 0) {
          left_seg->Merge(right_seg, pool_addr);
        }
        auto reserve_item = Allocator::ReserveItem();
        TX_BEGIN(pool_addr) {
                pmemobj_tx_add_range_direct(reserve_item, sizeof(*reserve_item));
...
#ifdef AU
                segment_num +=
              2 * (sizeof(*reserve_item) + sizeof(left_seg->next) + 15) / 16;
#endif
  ...
  for (int i = 0; i < capacity;) {
    ...
    for (int j = 0; j < kNumBucket; ++j) {
      ...
      if (ret != NONE) {
        ...
        printf(
            "the segment is %d, the bucket is %d, the local depth = %lld, "
            "the "
            "pattern is %lld\n",
      ...
      if (ret != NONE) {
        ...
        printf(
            "the segment is %d, the bucket is %d, the local depth is %lld, "
            "the "
            "pattern is %lld\n",
            i, j, ss->local_depth, ss->pattern);
        return 0;
