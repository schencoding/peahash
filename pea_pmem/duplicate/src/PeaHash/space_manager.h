...
std::atomic<uint8_t> gl = 0;
thread_local uint8_t thread_id = (gl++);
...
constexpr uint32_t stash_num_mask = (1U << 31U) - 1;
...
struct SpaceManager {
 ...
 public:
  ...
  uint32_t stash_chunk_num;  // 31 lock bit; 30:0 num
  ...
  SpaceManager(const char *pool_name, size_t pool_size, uint64_t thread_num) {
    ...
    chunk_cap = *mapped_len / chunk_size;
  ...
  /**
   * @return -1: no need to get the lock; else: current stash chunk num;
   */
  int get_spin_lock(uint32_t array_id) {
    uint32_t new_value;
    uint32_t old_value;
    do {
      while (true) {
        old_value = __atomic_load_n(&stash_chunk_num, __ATOMIC_ACQUIRE);
        if ((old_value & stash_num_mask) > array_id) return -1;
        if (!(old_value & stash_lock_set)) {
          new_value = old_value & stash_lock_set;
          break;
        }
      }
    } while (!CAS(&stash_chunk_num, &old_value, new_value));
    return old_value;
  }
  ...
  void allocStashChunkId(uint32_t chunk_id) {
    ...
    int array_num = get_spin_lock(id_in_stash_chunk_array);
    // CAS lock until success, load old_stash_chunk_num
    if (array_num >= 0) {
      for (int i = array_num; i <= id_in_stash_chunk_array; ++i) {
        uint32_t *stash_chunk_id = stash_chunk_id_array + i;
        *stash_chunk_id = allocChunk();
        auto stash_seg = (long long int *)chunk_addr(*stash_chunk_id);
      ...
      __atomic_store_n(&stash_chunk_num, id_in_stash_chunk_array + 1,
                       __ATOMIC_RELEASE);
    }
  }
