...
void Run() {
  ...
  void *insert_workload = malloc(operation_num * sizeof(uint64_t));
  ...
  if( (ins_stream = fopen(insert_pth.data(), "r" )) != NULL ) {
    size_t num_ins = fread(insert_workload, sizeof(uint64_t), operation_num, ins_stream);
  ...
  if (operation == "skew-full") { /*do the benchmark for all single operations*/
    ...
    // index->getNumber();
    rarray[0].begin =  0;
    rarray[0].end = SAMPLE_NUM;
    // read to rarray[0].workload
    FILE *ss_stream;
    if( (ss_stream = fopen(skew_sp_pth.data(), "r" )) != NULL ) {
      size_t num_ss = fread(insert_workload, sizeof(uint64_t), SAMPLE_NUM, ss_stream);
    ...
    // skew search
    GeneralBench<T>(rarray, index, thread_num, SAMPLE_NUM, "skew_search",
                        &concurr_search_without_epoch);  

    FILE *us_stream;
    if( (us_stream = fopen(uniq_sp_pth.data(), "r" )) != NULL ) {
      size_t num_us = fread(insert_workload, sizeof(uint64_t), SAMPLE_NUM, us_stream);
      printf( "Number of items to search = %ld\n", num_us );
      fclose(us_stream);
    } else {
      printf( "File could not be opened\n" );
    ...
    GeneralBench<T>(rarray, index, thread_num, SAMPLE_NUM, "uniq_search",
                        &concurr_search_without_epoch); 
    
    shuffle((uint64_t *)insert_workload, SAMPLE_NUM);

    GeneralBench<T>(rarray, index, thread_num, SAMPLE_NUM, "Delete",
