...
const constexpr size_t kNumBucket =
    2048; /* the number of normal buckets in one segment 64*/
...
#ifdef AU
          segment_num += 2*(sizeof(*reserve_item) + sizeof(dir) + sizeof(back_dir) + 15) / 16;
#endif
...
#ifdef AU
          segment_num += 2 * (sizeof(*reserve_item) + sizeof(dir) + sizeof(back_dir) + sizeof(old_b->local_depth) + 15) / 16;
#endif
          old_b->local_depth += 1;
          Allocator::Free(reserve_item, dir);
  ...
  if (depth_diff == 0) {
    if (x % 2 == 0) {
      TX_BEGIN(pool_addr) {
              ...
              pmemobj_tx_add_range_direct(&old_b->local_depth,
                                          sizeof(old_b->local_depth));
#ifdef AU
              segment_num += 2*(15 + sizeof(Table<T> *) + sizeof(old_b->local_depth))/16;
#endif
    ...
    TX_BEGIN(pool_addr) {
            ...
            pmemobj_tx_add_range_direct(&old_b->local_depth,
                                        sizeof(old_b->local_depth));
#ifdef AU
            segment_num += 2 * (15 + sizeof(Table<T> *) * base + sizeof(old_b->local_depth)) / 16;
#endif
...
#ifdef AU
                segment_num += 2 * (sizeof(*reserve_item) + sizeof(left_seg->next) + 15) / 16;
#endif
