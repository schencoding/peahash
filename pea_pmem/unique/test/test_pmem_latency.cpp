...
DEFINE_string(index, "dash-16",
              "the index to evaluate:pea/dash-16/dash-512/cceh/level");
...
DEFINE_uint64(p, 200000000,
              "the number of operations(insert/search/deletion) to execute");
DEFINE_string(
    op, "full",
    "which type of operation to execute:ins/pos/neg/del/mixed/skew-all/full/skew-search");
...
Hash<T> *InitializeIndex(int seg_num) {
  ...
  if (index_type == "dash-16") {
    std::cout << "Initialize Dash-16" << std::endl;
    ...
    std::cout << "Initialize Dash-512" << std::endl;
...
void print_stat(double clock_circle) {
  double time_ns = (double(clock_circle - 15)) / 2.3;       // 15 is the overhead of rdtsc
  std::cout << time_ns << ",";
}

std::vector<uint64_t> ins_circle(0);
std::vector<uint64_t> pos_circle(0);
std::vector<uint64_t> neg_circle(0);
std::vector<uint64_t> del_circle(0);

void printVecStats(const std::vector<uint64_t> &data){
...
void printFourStats() {
  if (operation == "ins") {
    printVecStats(ins_circle);
    ins_circle.clear();
  } else if (operation == "pos") {
    printVecStats(pos_circle);
    pos_circle.clear();
  } else if (operation == "neg") {
    printVecStats(neg_circle);
    neg_circle.clear();
  } else if (operation == "del") {
    printVecStats(del_circle);
    del_circle.clear();
  }
}

void shuffle(uint64_t *workload, uint64_t number, uint64_t seed) {
...
void Run() {
  ...
  struct range *rarray;
  uint16_t partition_num = 20;
  uint64_t chunk_size = operation_num / partition_num;
  rarray = reinterpret_cast<range *>(malloc(partition_num * (sizeof(range))));
  ...
  if (operation == "recovery") {
    ...
    if(operation == "ins") {
      for (size_t j = 0; j < 100; j++) {
        // insert 1
        T insert_key = keys[operation_num + 110 + j];
        begin = rdtsc();   
        index->Insert(insert_key, DEFAULT);
        end = rdtsc();   
        total = end - begin; // total即 cpu clock cycle 
        ins_circle.push_back(total);
      }
    }

    shuffle(array, 100, 6284532);
    if (operation == "pos") {
      for (size_t j = 0; j < 100; j++) {
        // pos 1
        T pos_key = keys[operation_num + 110 + array[j]];
        begin = rdtsc();
        index->Get(pos_key);
        end = rdtsc();
        total = end - begin;
        pos_circle.push_back(total);
      }
    }
    ...
    if (operation == "neg") {
      for (size_t j = 0; j < 100; j++) {
        // neg 1
        T neg_key = keys[operation_num + 5 + j];
        begin = rdtsc();
        index->Get(neg_key);
        end = rdtsc();   
        total = end - begin;
        neg_circle.push_back(total);
      }
    }

    shuffle(array, 100, 1026771);
    if (operation == "del") {
      for (size_t j = 0; j < 100; j++) {
        // del1
        T del_key = keys[operation_num + 110 + array[j]];    
        begin = rdtsc();
        index->Delete(del_key);
        end = rdtsc();   
        total = end - begin;
        del_circle.push_back(total);
      }
    }
    printFourStats();

    uint64_t offset = generate_num - (rand() % 200000);
    uint64_t* random_gen = reinterpret_cast<uint64_t *>(workload);
    for (int i = 0; i < partition_num; i++)
    {
      concurr_insert_without_epoch(rarray + i, index);
      std::cout << (i + 1) * chunk_size << std::endl;
      // prepare environment
      for (size_t j = 0; j < 100; j++) {
        uint64_t rand64 = random_gen[--offset];

        if (operation == "ins") {
          // insert 1
          T insert_key = keys[operation_num + 110 + rand64 % 4000];
          begin = rdtsc();   
          index->Insert(insert_key, DEFAULT);
          end = rdtsc();   
          total = end - begin; // total即 cpu clock cycle 
          ins_circle.push_back(total);
          index->Delete(insert_key);
        }

        if (operation == "neg") {
          // neg 1
          rand64 = random_gen[--offset];
          T neg_key = keys[operation_num + 4628 + rand64 % 4000];
          begin = rdtsc();
          index->Get(neg_key);
          end = rdtsc();   
          total = end - begin;
        ...
        if (operation == "del") {
          // del1
          rand64 = random_gen[--offset];
          T del_key = keys[i * chunk_size + chunk_size - 1 - (rand64 % chunk_size)];     // the last one
          begin = rdtsc();
          index->Delete(del_key);
          end = rdtsc();   
          total = end - begin;
          del_circle.push_back(total);
          index->Insert(del_key, DEFAULT);
        }

        if (operation == "pos") {
          // pos 1
          rand64 = random_gen[--offset];
          T pos_key = keys[i * chunk_size + (rand64 % chunk_size)];
          begin = rdtsc();
          index->Get(pos_key);
          end = rdtsc();
          total = end - begin;
