...
DEFINE_double(r, 0.5, "read ratio for mixed workload:0~1.0");
DEFINE_double(s, 0.5, "insert ratio for mixed workload: 0~1.0");
...
void *insert_workload; // loaded keys
...
void mixed_without_epoch(struct range *_range, Hash<T> *index) {
  ...
  uint64_t begin = _range->begin;
  uint64_t end = _range->end;
  T *get_array = reinterpret_cast<T *>(insert_workload);
